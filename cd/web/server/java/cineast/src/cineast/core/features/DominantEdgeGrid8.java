...
public class DominantEdgeGrid8 extends AbstractFeatureModule {
	...
	private static final double MAX_DIST = 6026f * 2f;
	
	@Override
	public void init(PersistencyWriter<?> phandler) {
		this.phandler = phandler;
		this.phandler.open("features.DominantEdgeGrid8");
	}
	...
	public List<LongDoublePair> getSimilar(FrameContainer qc) {
		...
		short[] dominant = getDominants(edgeHist);
		FloatVector fv = new FloatVectorImpl(dominant);
		ResultSet rset = this.selector.select("SELECT * FROM features.DominantEdgeGrid8 USING DISTANCE MINKOWSKI(2)(\'" + fv.toFeatureString() + "\', edges) ORDER USING DISTANCE LIMIT " + limit);
	...
	public List<LongDoublePair> getSimilar(long shotId) {
		int limit = Config.resultsPerModule();		
		ResultSet rset = this.selector.select("WITH q AS (SELECT edges FROM features.DominantEdgeGrid8 WHERE shotid = " + shotId + ") SELECT shotid FROM features.DominantEdgeGrid16, q USING DISTANCE MINKOWSKI(2)(q.edges, DominantEdgeGrid16.edges) ORDER USING DISTANCE LIMIT " + limit);
		ArrayList<LongDoublePair> result = new ArrayList<>(limit);
		if(rset != null){
			try {
				while(rset.next()){
