...
public class EdgeARP88Full extends AbstractFeatureModule {
	...
	public void init(PersistencyWriter<?> phandler) {
		...
		this.phandler.open("features.EdgeARP88Full");
	...
	public List<LongDoublePair> getSimilar(FrameContainer qc) {
		...
		ResultSet rset = this.selector.select("SELECT * FROM features.EdgeARP88Full USING DISTANCE MINKOWSKI(2)(\'" + query.toFeatureString() + "\', arp) ORDER USING DISTANCE LIMIT " + limit);
	...
	public List<LongDoublePair> getSimilar(long shotId) {
		...
		ResultSet rset = this.selector.select("WITH q AS (SELECT arp FROM features.EdgeARP88Full WHERE shotid = " + shotId + ") SELECT shotid FROM features.EdgeARP88Full, q USING DISTANCE MINKOWSKI(2)(q.arp, EdgeARP88Full.arp) ORDER USING DISTANCE LIMIT " + limit);
		ArrayList<LongDoublePair> result = new ArrayList<>(limit);
		if(rset != null){
			try {
				while(rset.next()){
