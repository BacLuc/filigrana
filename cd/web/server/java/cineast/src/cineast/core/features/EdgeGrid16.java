...
public class EdgeGrid16 extends AbstractFeatureModule {
	...
	public void init(PersistencyWriter<?> phandler) {
		...
		this.phandler.open("features.EdgeGrid16");
...
//			}
	...
	public List<LongDoublePair> getSimilar(FrameContainer qc) {
		...
		ResultSet rset = this.selector.select("SELECT * FROM features.EdgeGrid16 USING DISTANCE MINKOWSKI(2)(\'" + query.toFeatureString() + "\', grid) ORDER USING DISTANCE LIMIT " + limit);
	...
	public List<LongDoublePair> getSimilar(long shotId) {
		...
		ResultSet rset = this.selector.select("WITH q AS (SELECT grid FROM features.EdgeGrid16 WHERE shotid = " + shotId + ") SELECT shotid FROM features.EdgeGrid16, q USING DISTANCE MINKOWSKI(2)(q.grid, EdgeGrid16.grid) ORDER USING DISTANCE LIMIT " + limit);
		ArrayList<LongDoublePair> result = new ArrayList<>(limit);
		if(rset != null){
			try {
				while(rset.next()){
