 ...
 *
 */
public class EHD extends AbstractFiligranaFeatureModule {
	...
	public List<LongDoublePair> getSimilar(FrameContainer qc) {
		...
		ResultSet rset = this.getSimilar(this.tablename,query, "hist" ,qc.getWatermark());
	...
	public List<LongDoublePair> getSimilar(long shotId) {
		...
		ResultSet rset = this.selector.select("WITH q AS (SELECT hist FROM "+this.tablename+" WHERE "+this.idField+" = " + shotId + ") SELECT "+this.idField+" FROM "+this.tablename+", q USING DISTANCE MINKOWSKI(2)(q.hist, "+this.tablename+".hist) ORDER USING DISTANCE LIMIT " + limit);
		ArrayList<LongDoublePair> result = new ArrayList<>(limit);
		if(rset != null){
			try {
				while(rset.next()){
