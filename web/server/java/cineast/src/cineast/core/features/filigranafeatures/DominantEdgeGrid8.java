...
public class DominantEdgeGrid8 extends AbstractFiligranaFeatureModule {

	private static final Logger LOGGER = LogManager.getLogger();
	private static final double MAX_DIST = 6026f * 2f;
	
	public DominantEdgeGrid8(){
		this.tablename="DominantEdgeGrid8";
	}
	@Override
	...
	public List<LongDoublePair> getSimilar(FrameContainer qc) {
		...
		short[] dominant = getDominants(edgeHist);
		FloatVector fv = new FloatVectorImpl(dominant);
		ResultSet rset = this.getSimilar(this.tablename,fv, "edges", qc.getWatermark());
	...
	public List<LongDoublePair> getSimilar(long shotId) {
		int limit = Config.resultsPerModule();		
		ResultSet rset = this.selector.select("WITH q AS (SELECT edges FROM "+this.tablename+" WHERE "+this.idField+" = " + shotId + ") SELECT "+this.idField+" FROM "+this.tablename+", q USING DISTANCE MINKOWSKI(2)(q.edges, "+this.tablename+".edges) ORDER USING DISTANCE LIMIT " + limit);
		ArrayList<LongDoublePair> result = new ArrayList<>(limit);
		if(rset != null){
			try {
				while(rset.next()){
