...
public class EdgeGrid16Full extends AbstractFiligranaFeatureModule {

	private static final Logger LOGGER = LogManager.getLogger();
	private static final float MAX_DIST = (float) (16f * MathHelper.SQRT2);
	
	public EdgeGrid16Full(){
		this.tablename="EdgeGrid16Full";
	}
	@Override
	...
	public List<LongDoublePair> getSimilar(FrameContainer qc) {
		...
		ResultSet rset = this.getSimilar(this.tablename,query, "grid",qc.getWatermark());
	...
	public List<LongDoublePair> getSimilar(long shotId) {
		...
		ResultSet rset = this.selector.select("WITH q AS (SELECT grid FROM "+this.tablename+" WHERE "+this.idField+" = " + shotId + ") SELECT "+this.idField+" FROM "+this.tablename+", q USING DISTANCE MINKOWSKI(2)(q.grid, "+this.tablename+".grid) ORDER USING DISTANCE LIMIT " + limit);
		ArrayList<LongDoublePair> result = new ArrayList<>(limit);
		if(rset != null){
			try {
				while(rset.next()){
